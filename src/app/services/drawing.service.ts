...
export class DrawingService {
  ...
  public handleMouseUp(options: any): void {
    ...
    // Handle Move Component Mode
    if (this.lineDrawingService.drawingMode === 'moveComponent') {
      this.lineDrawingService.handleMoveComponentMouseUp(this.canvas, options);
      return;
    }
  }
  ...
  public handleMouseMove(options: any): void {
    ...
    // Handle Move Component Mode
    if (this.lineDrawingService.drawingMode === 'moveComponent') {
      this.lineDrawingService.handleMoveComponentMouseMove(this.canvas, options);
      return;
    }
    ...
    // Handle T-piece hover in all modes
    this.handleGeneralTeeHover(options);
  ...
  public cancelDrawing(): void {
    ...
    // Reset T-piece highlights
    this.resetGeneralTeeHighlight();
  ...
  private handleGeneralTeeHover(options: any): void {
    if (!this.canvas) return;

    // Skip if we're in a mode that already handles T-piece hover
    if (this.pipingService.isActive() || this.lineDrawingService.drawingMode === 'moveComponent') {
      return;
    }

    const pointer = this.canvas.getPointer(options.e);
    
    // Find T-piece objects near mouse
    ...
    for (const teeObj of teeObjects) {
      ...
      // Check if mouse is over expanded area
      if (pointer.x >= expandedBounds.left && 
        ...
        // Calculate distance to T-piece center
  ...
  private highlightGeneralTee(teeRect: fabric.Rect): void {
    ...
    teeLines.forEach((line: any) => {
      if (line && typeof line.set === 'function') {
        ...
        line.set('stroke', '#4CAF50'); // Green hover color
      }
    });

    // Make selection rectangle slightly visible
    teeRect.set({
  ...
  private resetGeneralTeeHighlight(): void {
    if (!this.canvas || !this.hoveredTee) return;

    // Reset T-piece lines to original color
    const teeLines = (this.hoveredTee as any).teeLines || [];
    teeLines.forEach((line: any) => {
